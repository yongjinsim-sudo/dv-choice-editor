...
const activeEnvironmentKey = 'dvChoiceEditor.activeEnvironment';
...
function getConfiguredEnvironments(): EnvironmentProfile[] {
	const config = vscode.workspace.getConfiguration('dvChoiceEditor');
...
async function pickEnvironment(context: vscode.ExtensionContext, forcePick: boolean): Promise<EnvironmentProfile | undefined> {
	...
	if (!environments.length) {
		const choice = await vscode.window.showInformationMessage(
			'DV Choice Editor: No Dataverse environments configured yet.',
			'Set Up Environment',
			'Cancel'
		);
	...
	const accessToken = await vscode.window.withProgress(
		{
			...
			title: `DV Choice Editor: Connecting to ${profile.name}`,
			cancellable: false
		},
